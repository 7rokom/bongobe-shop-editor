...
const ResellerPayments = () => {
  ...
  const approvedPayments = requests.filter((p) => p.status === 'অনুমোদিত').reduce((sum, p) => sum + p.amount, 0);
  const pendingPayments = requests.filter((p) => p.status === 'পেন্ডিং').reduce((sum, p) => sum + p.amount, 0);
  const totalBalance = deliveredProfit - returnLoss - approvedPayments - pendingPayments;
  const withdrawable = Math.max(0, totalBalance - MINIMUM_BALANCE);

  useEffect(() => {
    if (resellerId) {
      db.from('reseller_payment_methods').select('*').eq('reseller_id', resellerId).then(({ data }) => {
