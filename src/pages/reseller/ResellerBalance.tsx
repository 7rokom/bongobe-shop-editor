...
const ResellerBalance = () => {
  ...
  const returnOrders = orders.filter((o) => o.status === 'রিটার্ন');

  const deliveredProfit = deliveredOrders.reduce((sum, o) => sum + o.totalProfit, 0);
  const returnLoss = returnOrders.reduce((sum, o) => sum + (o.deliveryCharge || 0) + (o.packagingCharge || 0), 0);
  const pendingProfit = orders
    .filter((o) => o.status !== 'ডেলিভারড' && o.status !== 'ক্যান্সেল' && o.status !== 'রিটার্ন')
    .reduce((sum, o) => sum + o.totalProfit, 0);
  const totalWithdrawn = paymentRequests.filter((p) => p.status === 'অনুমোদিত').reduce((sum, p) => sum + p.amount, 0);
  const pendingPayments = paymentRequests.filter((p) => p.status === 'পেন্ডিং').reduce((sum, p) => sum + p.amount, 0);
  const withdrawable = deliveredProfit - returnLoss - totalWithdrawn - pendingPayments;

  const [expandedOrder, setExpandedOrder] = useState<string | null>(null);

  const stats = [
    { title: 'ডেলিভারড লাভ', value: `৳${deliveredProfit}`, icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-500/10' },
    { title: 'রিটার্ন লস', value: `-৳${returnLoss}`, icon: TrendingDown, color: 'text-red-600', bg: 'bg-red-500/10' },
  ...
  const profitLossOrders = orders.filter((o) => o.status === 'ডেলিভারড' || o.status === 'রিটার্ন');
          ...
          {profitLossOrders.length === 0 ? (
            ...
            <div className="space-y-2">
              {profitLossOrders.map((o) => {
                ...
                const isReturn = o.status === 'রিটার্ন';
                ...
                const returnLossAmount = delivery + packaging;
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <span className={`text-sm font-bold ${isDelivered ? 'text-emerald-600' : 'text-red-600'}`}>
                          {isDelivered ? `+৳${profit}` : `-৳${returnLossAmount}`}
                        </span>
                        {isExpanded ? (
                      ...
                      <div className="border-t px-3 pb-3 pt-2 bg-muted/20">
                        ...
                        <div className="rounded-lg bg-card border p-3 space-y-1.5 text-xs">
                          {isDelivered ? (
                            ...
                            <>
                              <p className="text-[11px] text-muted-foreground mb-1">রিটার্নে শুধুমাত্র ডেলিভারি ও প্যাকেজিং চার্জ কাটা হয়েছে</p>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">- ডেলিভারি চার্জ:</span>
                                <span className="text-red-500">-৳{delivery}</span>
                              </div>
