...
const ResellerDashboard = () => {
  ...
  const returnOrders = filteredOrders.filter((o) => o.status === 'রিটার্ন');
  ...
  const allDeliveredProfit = allOrders
    .filter((o) => o.status === 'ডেলিভারড')
    .reduce((s, o) => s + o.totalProfit, 0);
  const pendingPayments = allPayments
    .filter((p) => p.status === 'পেন্ডিং')
    .reduce((s, p) => s + p.amount, 0);
  const withdrawable = allDeliveredProfit - totalWithdrawn - pendingPayments;
