...
import { Order } from "@/components/admin/OrderDetailDialog";
...
const buildTimeline = (status: string) => {
  const isCancel = status === 'ক্যান্সেল';
  const isReturn = status === 'রিটার্ন';
...
const OrderTracking = () => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    let found: Order[];
    if (searchType === "order") {
      found = orders.filter((o) => o.id.toLowerCase().includes(q.toLowerCase()));
    } else {
      found = orders.filter((o) => o.phone.includes(q));
    }
      ...
      <body>
        ...
        <div class="info-grid">
          ...
          <div class="info-box">
            ...
            <p><strong>BongoBe Multi Online Shop</strong></p>
            <p>মানিকনগর পুকুর পাড়, মুগদা, ঢাকা</p>
            <p>ফোন: 01948818255</p>
          </div>
        </div>
        <table>
          <thead><tr><th>#</th><th>পণ্যের নাম</th><th>পরিমাণ</th><th>একক মূল্য</th><th>মোট</th></tr></thead>
    ...
    <div className="py-8 bg-background min-h-screen">
      <div className="container-box max-w-3xl">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[17px] text-muted-foreground">অর্ডার নম্বর অথবা ফোন নম্বর দিয়ে অর্ডারের অবস্থা জানুন</p>
        </div>

        {/* Search Type Toggle */}
        <div className="flex gap-2 mb-3">
          <Button variant={searchType === "order" ? "default" : "outline"} size="sm" className="rounded-[5px] text-[15px] gap-1" onClick={() => { setSearchType("order"); setQuery(""); }}>
            <Package className="h-4 w-4" /> অর্ডার নম্বর
          </Button>
          <Button variant={searchType === "phone" ? "default" : "outline"} size="sm" className="rounded-[5px] text-[15px] gap-1" onClick={() => { setSearchType("phone"); setQuery(""); }}>
        ...
        <form onSubmit={handleSearch} className="flex gap-2 mb-8">
          <div className="flex-1 relative">
            ...
            <Input
              placeholder={searchType === "order" ? "অর্ডার নম্বর লিখুন (যেমন: #1001)" : "ফোন নম্বর লিখুন (যেমন: 01712345678)"}
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              className="pl-10 h-12 text-[17px] rounded-[5px] border-2 border-primary/30 focus:border-primary"
            />
          </div>
          <Button type="submit" className="h-12 px-6 gap-2 text-[17px] rounded-[5px]">
        ...
        {notFound && (
          <div className="border-2 border-destructive/30 bg-destructive/5 rounded-[5px] p-6 text-center space-y-2">
            ...
            <p className="text-[15px] text-muted-foreground">সঠিক {searchType === "order" ? "অর্ডার নম্বর" : "ফোন নম্বর"} দিয়ে আবার চেষ্টা করুন।</p>
            <div className="flex justify-center gap-3 pt-2">
              <a href="tel:01948818255"><Button size="sm" variant="outline" className="gap-1 rounded-[5px] text-[15px]"><Phone className="h-4 w-4" /> কল করুন</Button></a>
              <a href="https://wa.me/8801948818255" target="_blank" rel="noopener noreferrer"><Button size="sm" variant="outline" className="gap-1 rounded-[5px] text-[15px]"><MessageCircle className="h-4 w-4" /> WhatsApp</Button></a>
            </div>
          </div>
        )}
        ...
        {searchedOrders.length > 1 && !selectedOrder && (
          <div className="space-y-3">
            ...
            {searchedOrders.map((o) => {
              ...
              return (
                <button key={o.id} onClick={() => setSelectedOrder(o)} className="w-full border-2 border-primary/20 rounded-[5px] p-4 flex items-center justify-between hover:border-primary transition-colors text-left">
                  <div>
                    <p className="text-[16px] font-bold text-primary">{o.id}</p>
          ...
          <div className="space-y-5" ref={invoiceRef}>
            ...
            <div className="border-2 border-primary/20 rounded-[5px] p-5 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <p className="text-[15px] text-muted-foreground">অর্ডার নম্বর</p>
              ...
              <div className="border-2 border-primary/20 rounded-[5px] p-5 space-y-4">
                ...
                <div className="pt-3 border-t border-border">
                  ...
                  <div className="flex gap-2">
                    <a href="tel:01948818255" className="flex-1">
                      <Button className="w-full gap-2 rounded-[5px] text-[15px]" size="sm"><Phone className="h-4 w-4" /> কল করুন</Button>
                    </a>
                    <a href="https://wa.me/8801948818255" target="_blank" rel="noopener noreferrer" className="flex-1">
        ...
        {searchedOrders.length === 0 && !notFound && (
          <div className="text-center text-muted-foreground space-y-2 py-8">
            ...
            <p className="text-[17px]">উপরে আপনার অর্ডার নম্বর বা ফোন নম্বর লিখে সার্চ করুন</p>
            <p className="text-[14px]">অর্ডার নম্বর <span className="font-bold text-primary">#1001</span> বা ফোন নম্বর <span className="font-bold text-primary">01712345678</span> দিয়ে টেস্ট করুন</p>
          </div>
        )}
      </div>
    </div>
