...
const IncompleteOrders = () => {
  ...
  const renderCancelledTable = () => (
    <>
      {selectedCancelled.size > 0 && (
        <div className="flex items-center gap-3 bg-destructive/5 border border-destructive/20 rounded-md p-3 mb-3">
          <span className="text-sm font-medium">{selectedCancelled.size}টি সিলেক্টেড</span>
          <Button variant="destructive" size="sm" className="h-8 gap-1" onClick={handleBulkDelete}><Trash2 className="w-3 h-3" /> মুছে ফেলুন</Button>
          <Button variant="outline" size="sm" className="h-8" onClick={() => setSelectedCancelled(new Set())}>বাতিল</Button>
        </div>
      )}
      {/* Desktop */}
      <Card className="border-0 shadow-sm hidden lg:block">
            ...
            <table className="w-full text-sm table-fixed">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="text-center py-3 px-2 font-medium text-muted-foreground w-[32px]">
                    <Checkbox checked={selectedCancelled.size === cancelledOrders.length && cancelledOrders.length > 0} onCheckedChange={toggleSelectAllCancelled} className="h-3.5 w-3.5" />
                  </th>
                  <th className="text-left py-3 px-4 font-medium text-muted-foreground w-[120px]">ইনভয়েজ</th>
                  <th className="text-left py-3 px-4 font-medium text-muted-foreground w-[200px]">কাস্টমার</th>
                  <th className="text-left py-3 px-4 font-medium text-muted-foreground w-[180px]">প্রোডাক্ট</th>
              ...
              <tbody>
                {cancelledOrders.map(order => (
                  <tr key={order.id} className={`border-b last:border-0 hover:bg-muted/30 align-top ${selectedCancelled.has(order.id) ? 'bg-primary/5' : ''}`}>
                    <td className="py-3 px-2 text-center"><Checkbox checked={selectedCancelled.has(order.id)} onCheckedChange={() => toggleCancelledSelect(order.id)} className="h-3.5 w-3.5" /></td>
                    <td className="py-3 px-4">
                      <span className="font-bold text-primary text-xs">{order.id}</span>
                      <p className="text-[10px] text-muted-foreground mt-0.5">{new Date(order.date).toLocaleDateString('bn-BD')}</p>
      ...
      {/* Mobile */}
      <div className="lg:hidden space-y-3">{cancelledOrders.map(o => renderMobileCard(o, false, true))}</div>
    </>
  );
