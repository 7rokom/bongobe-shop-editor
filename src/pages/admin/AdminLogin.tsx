...
const AdminLogin = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    // Try admin login first, then employee
    if (login(email, password)) {
      navigate('/admin');
    } else if (loginEmployee(email, password, employees)) {
      navigate('/admin');
    } else {
      ...
      <Card className="w-full max-w-md shadow-xl border-0">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full h-12 text-base font-semibold">লগইন করুন</Button>
          </form>
        </CardContent>
      </Card>
    </div>
