...
import { Plus, Search, Edit2, Trash2, ArrowLeft, Upload, Link, X, Image, Save, Eye, FileText, StickyNote, ExternalLink, Copy, Download } from 'lucide-react';
...
const BlogAdmin = () => {
  ...
  const openNewEditor = (type: ContentType) => {
    setEditing(null);
    setFormData({ title: '', content: '', category: '', metaDescription: '', metaKeywords: '' });
    setStatus('published');
    setFeaturedImage('');
    setActiveTab(type);
    setShowEditor(true);
  ...
  const handleSave = (saveStatus: 'published' | 'draft') => {
    ...
    if (editing) {
      updatePost(editing.id, postData);
              ...
              <div className="flex gap-3 sm:gap-4">
                ...
                <div className="flex items-start gap-1 shrink-0 flex-wrap">
                  ...
                  <Button
                    ...
                    onClick={() => { deletePost(post.id); toast.success('ডিলিট হয়েছে'); }}
                  >
