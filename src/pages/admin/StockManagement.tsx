...
// Build a map: stockProductName → list of product titles that use it
const useStockProductMap = () => {
  const products = useProductStore((s) => s.products);
  return useMemo(() => {
    const map: Record<string, string[]> = {};
    products.forEach(p => {
      if (p.stockType === 'self' && p.stockProductName) {
        if (!map[p.stockProductName]) map[p.stockProductName] = [];
        map[p.stockProductName].push(p.title);
      }
    });
    return map;
  }, [products]);
...
const StockManagement = () => {
  ...
  const stockProductMap = useStockProductMap();
  ...
  // Stock calculation from orders
  const shipmentStatuses = ['প্যাকেজিং', 'শিপমেন্ট', 'ডেলিভারির পথে'];
  const deliveredStatus = 'ডেলিভারড';
  const returnStatuses = ['রিটার্ন', 'পেইড রিটার্ন'];

  const stockCalc = useMemo(() => {
    const titleToStock: Record<string, string> = {};
    Object.entries(stockProductMap).forEach(([stockName, titles]) => {
      titles.forEach(t => { titleToStock[t] = stockName; });
    });
    ...
    orders.forEach(o => {
      ...
      // Skip vendor stock orders — they don't affect self stock
      ...
      (o.items as any[]).forEach((item: any) => {
        ...
        if (shipmentStatuses.includes(status)) result[stockName].inShipment += qty;
        if (returnStatuses.includes(status)) result[stockName].returned += qty;
      });
    });
    ...
    return result;
  }, [orders, resellerOrders, stockProductMap, stockTypes]);
