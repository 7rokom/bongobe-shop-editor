  ...
  const sendToCarrybee = async (order: Order) => {
    ...
    try {
      ...
      const data = await callCarrybee({
        ...
        merchant_order_id: order.id.replace('#', ''), recipient_name: order.customer,
        recipient_phone: order.phone, recipient_address: order.address,
        city_id: cbSettings.defaultCityId || 14, zone_id: cbSettings.defaultZoneId || 5,
        collectable_amount: order.total, product_description: itemDesc,
