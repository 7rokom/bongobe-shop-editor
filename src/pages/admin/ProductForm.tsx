...
import { ArrowLeft, Upload, Link, X, Image, Video, Save, Eye, FileText, Star, Plus, Trash2 } from 'lucide-react';
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="space-y-2"><Label>কেনা দাম (৳)</Label><Input type="number" placeholder="0" value={formData.buyPrice} onChange={(e) => updateField('buyPrice', e.target.value)} /></div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {galleryImages.map((img, i) => (
                      <div key={i} className="relative">
                        <img src={img} alt={`Gallery ${i + 1}`} className="w-20 h-20 rounded-lg object-cover border" />
                        <button onClick={() => removeGalleryImage(i)} className="absolute -top-2 -right-2 bg-destructive text-destructive-foreground rounded-full p-0.5"><X className="w-3 h-3" /></button>
                      </div>
                    ))}
                  </div>
                )}
                        ...
                        {review.images && review.images.length > 0 && (
                          <div className="flex gap-1 mt-1">
                            {review.images.map((img, i) => (
                              <img key={i} src={img} alt="" className="w-10 h-10 object-cover rounded border border-border" />
                            ))}
                          </div>
                        )}
                      </div>
